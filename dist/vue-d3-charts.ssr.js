  ...
  _createClass(d3barchart, [{
    ...
    value: function enterElements() {
      ...
      var rects = newbars.selectAll('.chart__bar').data(function (d) {
        ...
        var label_key = _this3.cfg.tooltip.label;
        var value_key = _this3.cfg.values[i % _this3.cfg.values.length];

        _this3.tooltip.html(function () {
          return label_key ? "<div>".concat(d[label_key], "</div>") : "<div>".concat(value_key, ": ").concat(d[value_key], "</div>");
        }).classed('active', true);
      }).on('mouseout', function () {
        _this3.tooltip.classed('active', false);
      }).on('mousemove', function () {
        _this3.tooltip.style('left', window.event.offsetX + 'px').style('top', window.event.offsetY + 'px').style('transform', 'translate(-50%, -100%)');
      });
    }
    /**
     * Update chart's elements based on data change
